...
> 模块 03 - 记忆系统 | 第 5 节 | 前置：[VectorStore 记忆作为工具](./04-vectorstore-memory.md)
...
下一节 [多用户记忆隔离](./06-multi-user-isolation.md) 把这一切串起来——用 `thread_id` 在 checkpointer 里隔离会话，用 `namespace` 在 store 里隔离用户和租户。
