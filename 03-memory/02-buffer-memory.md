...
每个 checkpoint 都是一个不可变的状态快照。这给后面的"时间旅行"（回退到任意 checkpoint）打下基础——后续 [State、Channels 与 Checkpointer](../05-agent-architecture/04-langgraph-state.md) 会展开。
...
如果你希望更激进——比如旧消息直接压缩成摘要而不只是丢弃——那是下一节 [Summary 策略：middleware](./03-summary-memory.md) 的内容。
...
下一节 [Summary 策略：middleware](./03-summary-memory.md) 讲一种更聪明的策略——不只是裁剪，而是把旧消息**压缩成摘要**塞回 prompt。
