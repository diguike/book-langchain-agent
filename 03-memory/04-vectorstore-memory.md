...
> 模块 03 - 记忆系统 | 第 4 节 | 前置：[Summary 策略 - 用 middleware 压缩历史](./03-summary-memory.md)
...
下一节 [自定义后端：实现自己的 checkpointer / store](./05-custom-message-history.md) 讲怎么把 Redis 这类后端接进 LangChain 1.x 的 checkpointer / store 接口。
