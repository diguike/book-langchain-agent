...
> 模块 03 - 记忆系统 | 第 6 节 | 前置：[自定义后端 - 实现自己的 checkpointer 与 store](./custom-message-history.md)
...
1. [1.x 时代的记忆系统](./memory-overview.md) - 工作记忆 / checkpointer / store / tool 四层模型
2. [短期记忆 - thread-based checkpointer](./buffer-memory.md) - MemorySaver / PostgresSaver 与 trim middleware
3. [Summary 策略 - 用 middleware 压缩历史](./summary-memory.md) - `beforeModel` + `RemoveMessage`
4. [VectorStore 记忆作为工具](./vectorstore-memory.md) - 把"回忆"做成工具暴露给 Agent
5. [自定义后端 - 实现自己的 checkpointer 与 store](./custom-message-history.md) - Redis 版的两套接口
6. [多用户记忆隔离](./multi-user-isolation.md) - 本节

下一模块 [04-工具与函数调用](../04-tools/tool-interface.md) 引入 Agent 的"行动能力"。再下一模块 [05-Agent 架构](../05-agent-architecture/create-agent.md) 把记忆、工具、middleware 真正组装成完整的 Agent。
