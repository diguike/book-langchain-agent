...
模块之间的依赖关系如下图所示。箭头表示「学后者前应先掌握前者」，并排的模块可以并行学习：

```mermaid
graph TB
    M1["模块 1 核心抽象<br/>Model / Prompt / Parser"]
    M2["模块 2 Chain 组合"]
    M3["模块 3 记忆系统"]
    M4["模块 4 工具调用"]
    M5["模块 5 Agent 架构"]
...
图 1-1：模块依赖关系。模块 4（工具调用）是 Agent 的硬性前置，处在整张图的咽喉位置。
...
准备好后，先读 [前置知识清单](./02-prerequisites.md) 确认基础，再跟着 [环境搭建指南](./03-setup.md) 配好开发环境，就可以正式开始了。
