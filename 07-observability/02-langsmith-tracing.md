...
> 模块 07 - 可观测性与评估 | 前置知识：[Callback 系统](./01-callbacks.md)、[createAgent 入门](../05-agent-architecture/01-create-agent.md)
...
[流式输出深入](../05-agent-architecture/10-stream-modes.md) 讲过的 `stream` / `streamEvents`，在 trace 里也是自动记录的。流式调用会在 LangSmith 上呈现为一条完整 trace，每个 token 的产出虽然不展开但总耗时和总用量准确。
...
Dataset 的实际用法在下一节 [评估方法与指标](./03-evaluation.md) 展开。
...
下一节 [评估方法与指标](./03-evaluation.md) 讲怎么用 LangSmith 的 dataset 和 evaluator 系统化跑评估，把"感觉好像变好了"变成"correctness 从 0.78 提升到 0.86"。
