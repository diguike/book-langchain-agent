...
这种行为是合理的——一旦数据流被破坏，后续步骤继续跑只会污染日志。需要容错的场景请看 [Fallback 与重试](./05-fallback-retry.md)。
...
1. **流式输出**：`RunnableSequence` 原生支持 `.stream()`，但只有最后一步是真正流式的，中间步骤会等上一步完整产出。具体哪些节点会阻断流，下一节 [Streaming 流式输出](./04-streaming.md) 会专门讲。
...
下一节我讲 [RunnableParallel](./02-runnable-parallel.md)——同一个输入同时跑多个分支，把延迟从"步骤数 × 单步耗时"压到"max(单步耗时)"。
