...
数据流的拓扑是固定的——从 A 流向 B 流向 C，每个节点处理一次输入。状态机的拓扑是动态的——节点之间根据当前状态决定下一步去哪儿，可以回头、可以分叉、可以暂停。如图 2-7 所示，两者最直观的差别就是：LCEL 是一条不回头的直线，LangGraph 允许节点根据状态路由回前面。

```mermaid
graph LR
    subgraph LCEL["LCEL 数据流（固定拓扑）"]
        direction LR
        A1["A"] --> B1["B"] --> C1["C"]
    end
    subgraph LG["LangGraph 状态机（动态拓扑）"]
        direction LR
        A2["节点 A"] --> B2["节点 B"]
        B2 -->|条件| C2["节点 C"]
        B2 -->|条件回环| A2
...
<small>图 2-7：两种范式对比。LCEL 单向到底，LangGraph 可按状态回环、分叉、暂停</small>
