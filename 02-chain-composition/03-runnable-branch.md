...
`RunnableBranch` 就是 LCEL 里写 if-else 的标准方式。它接收一组 `[条件, 分支]` 对再加一个默认分支，跑的时候**只挑第一个命中的分支执行**——这是它和 `RunnableParallel` 的根本差别。如图 2-3 所示，条件按顺序求值，命中即走对应分支，全不命中走默认分支，永远只有一条路径被执行。
...
<small>图 2-3：RunnableBranch 的条件路由，条件顺序求值，只有第一个命中的分支被执行</small>
...
最后一点很重要：`02-Chain 组合` 这一模块讲的所有原语都是**无状态的流水线**。一旦你发现自己想要"循环"、"回滚"、"分支汇合到同一个状态"，那就是 LangGraph 的活儿，不该硬塞进 LCEL。详细的取舍标准看 [LCEL vs LangGraph 决策指南](./06-lcel-vs-langgraph.md)。
...
下一节我讲 [Streaming 流式输出](./04-streaming.md)——怎么让用户在 200ms 就看到模型开始打字，而不是等 5 秒钟拿到完整文本。
