...
> 模块 05 - Agent 架构 | 前置知识：[createAgent 入门](./01-create-agent.md)、[LangGraph 入门](./03-langgraph-intro.md)
...
前端用 `EventSource` 或 `fetch` + `ReadableStream` 消费。完整的生产部署细节在 [流式接口 SSE / WebSocket](../08-production/02-streaming-api.md)。
...
完整的部署细节（包括 WebSocket 备选方案、负载均衡时的 sticky session 配置）在 [流式接口 SSE / WebSocket](../08-production/02-streaming-api.md)。
