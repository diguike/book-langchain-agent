...
技巧：**用相同的 `id` 返回新 AIMessage，`messagesStateReducer` 会把旧消息替换掉**（这个行为我在 [LangGraph State](./langgraph-state.md) 里讲过）。比直接 mutate state 干净得多。
...
期间后端进程可以重启、扩容、迁移，因为所有 state 都在 Postgres 里。这是 [LangGraph State 与 Checkpointer](./langgraph-state.md) 里讲的"thread 持久化"在 HITL 上的直接应用。
...
[Multi-Agent](./multi-agent.md) 里的退款 Agent 一上来就该挂 interrupt——退款是不可逆操作。两种集成方式：
