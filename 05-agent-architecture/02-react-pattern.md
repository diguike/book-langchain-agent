...
我在 [createAgent 入门](./01-create-agent.md) 里说过，`createAgent` 内部就是一张 `model ↔ tools` 的 LangGraph 图。这张图跑的就是 ReAct 循环——模型节点输出一条带 `tool_calls` 的消息，工具节点执行、把结果回灌成 `ToolMessage`，模型节点再读这条新消息决定下一步。
...
更精细的流式选项——比如 token-by-token 推模型生成、或者按业务自定义事件——见 [流式输出深入](./10-stream-modes.md)。
...
如果某个任务经常撞到 25 步上限，那它就不该用 ReAct，应该改用 [Plan-and-Execute](./05-plan-and-execute.md)——先规划再执行，避免在子任务上空转。
...
调试这种问题最快的方式是接 [LangSmith](../07-observability/02-langsmith-tracing.md)，每一次调用都能可视化看到模型选了哪些工具。
...
- **5+ 步的复杂任务**：ReAct 没有全局视野，走弯路概率大。换 [Plan-and-Execute](./05-plan-and-execute.md)
- **输出质量要求高的写作**：单次生成质量不稳定，需要套一层 Critic。换 [Self-Reflection](./06-self-reflection.md)
- **多专家协作**：单个 Agent 工具太多 prompt 会爆炸。换 [Multi-Agent](./08-multi-agent.md)
- **高风险操作**：删数据、转账，必须有人工审批。换 [Human-in-the-Loop](./09-human-in-the-loop.md)
...
下一节 [Plan-and-Execute](./05-plan-and-execute.md) 处理 ReAct 处理不了的复杂多步任务。
