...
2. **替代 LangGraph 的拓扑设计**：如果你需要"先调 A，再根据 A 的结果决定调 B 还是 C"，这是图拓扑，应该用 [LangGraph](./langgraph-intro.md) 直接画图，不要在 middleware 里塞 if-else
...
下一节 [Multi-Agent 协作](./multi-agent.md) 把多个 Agent 串起来构成更大的系统。
