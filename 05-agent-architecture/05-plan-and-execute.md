...
[ReAct 模式](./02-react-pattern.md) 一节里我提过，ReAct 是贪心策略——每一步只看眼前。任务 2 到 4 步时这没问题，但一旦上了 5 步以上，三个问题集中爆发：
...
State 里只放最必要的字段。注意 `plan` 是覆盖型 reducer——replanner 整体替换剩余步骤；`pastSteps` 是追加型 reducer——每完成一步就 append 一条记录。这两种语义在 [LangGraph State 与 Checkpointer](./04-langgraph-state.md) 里有展开。
...
下一节 [Self-Reflection](./06-self-reflection.md) 讲另一种循环模式——让 Agent 自己评估输出、迭代改进。
