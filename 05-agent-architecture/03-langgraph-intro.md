---
title: LangGraph 入门
feishu_url: "https://fivwvysqdz.feishu.cn/wiki/XUhfwxbEGiYSnlkCl9ecj92KnDe"
last_synced: "2026-05-25T02:42:13+08:00"
---
...
我在 [createAgent 入门](./01-create-agent.md) 里讲过——`createAgent` 内部就是 LangGraph 跑的 `model ↔ tools` 状态机。日常 90% 的 Agent 场景靠 `createAgent` + middleware 就够了。**剩下 10% 的复杂场景**，需要你自己画图：

- 多 Agent 协作的路由拓扑（[Multi-Agent](./08-multi-agent.md)）
- Plan / Execute / Reflect 这类多节点循环（[Plan-and-Execute](./05-plan-and-execute.md)、[Self-Reflection](./06-self-reflection.md)）
- 并发分支（同时搜多个数据源）
- 显式的人工审批断点（[Human-in-the-Loop](./09-human-in-the-loop.md)）
- 自定义状态字段（[State 与 Checkpointer](./04-langgraph-state.md)）
...
没 reducer 又有多个并发节点写同一字段，结果不确定，所以**共享字段必须配 reducer**。`reducer` 的设计细节、坑、最佳实践都在 [LangGraph State 与 Checkpointer](./04-langgraph-state.md) 展开。
...
这种"把 `createAgent` 当 LangGraph 节点用"的写法在 [Plan-and-Execute](./05-plan-and-execute.md) 和 [Multi-Agent](./08-multi-agent.md) 都有完整示例。
...
await app.invoke(input, {
  ...
  // 持久化用的会话 ID（配合 checkpointer，见 04-langgraph-state.md）
...
下一节 [LangGraph State 与 Checkpointer](./04-langgraph-state.md) 深入 state 设计、reducer 用法、和把 graph 状态持久化到 SQLite / Postgres 的方法。
