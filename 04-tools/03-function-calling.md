...
> 模块 04 - 工具与函数调用 | 前置：[Tool 接口与定义](./01-tool-interface.md)、[自定义 Tool 开发](./02-custom-tool.md) | 后续：[MCP Server 集成](./04-mcp-server.md)
...
2. **没有手写循环**——`createAgent` 内部跑了 LangGraph 的 `model ↔ tools` 循环，详见 [createAgent 入门](../05-agent-architecture/01-create-agent.md)。
...
`createAgent` 内部默认 `auto`，需要其他策略时可以通过 middleware 调整（详见 [Middleware 系统](../05-agent-architecture/07-middleware.md)）。
...
下一节 [MCP Server 集成](./04-mcp-server.md) 把工具来源从"应用内部"扩展到"外部 MCP Server"，接入整个 MCP 生态。
