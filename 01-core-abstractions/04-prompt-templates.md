...
在 [createAgent 入门](../05-agent-architecture/01-create-agent.md) 一节你会看到 Agent 的 `systemPrompt` 直接传字符串，不走 `ChatPromptTemplate`：
...
需要动态 system prompt（按用户 tier 切换内容）时也不在外层做模板渲染，而是用 `dynamicSystemPromptMiddleware` 拿到运行时的 `state` 和 `context` 来生成。详见 [Middleware 系统](../05-agent-architecture/07-middleware.md)。
...
下一节进入 [Output Parsers](./05-output-parsers.md)，看怎么把模型输出从自由文本变成结构化数据。
