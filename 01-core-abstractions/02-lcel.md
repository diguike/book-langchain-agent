...
> 模块 01 - 核心抽象 | 前置：[Runnable 接口详解](./01-runnable-interface.md)
...
`streamEvents` 是更细粒度的版本，适合调试和监控，详见 [Runnable 接口详解](./01-runnable-interface.md)。
...
`createAgent` 内部就是用 LangGraph 实现的循环。详见 [createAgent 入门](../05-agent-architecture/01-create-agent.md)。
...
下一节进入 [Model I/O](./03-model-io.md)，看 LCEL 的核心节点——Chat Model 是怎么工作的。
