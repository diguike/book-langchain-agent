...
生产部署要把 `MemorySaver` 换成 `PostgresSaver`（[Memory / Checkpointer](../05-agent-architecture/04-langgraph-state.md) 有完整写法），否则进程重启会丢对话历史。
...
下一节[代码助手 Agent](./02-code-assistant.md) 换个场景：单 Agent + 大量工具 + 流式 UI。
