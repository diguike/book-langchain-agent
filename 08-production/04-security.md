...
> 模块 08 - 生产部署 | 前置知识：[Tool 接口与定义](../04-tools/01-tool-interface.md)、[API 服务化](./01-api-server.md)
...
这一节按"输入侧 → 执行侧 → 输出侧 → 运维侧"四层讲怎么设防。每一层都不是万能的，要叠起来用——任何一层漏掉都可能被绕过。四层各自拦什么，如图 8-7 所示。
...
graph TB
    ...
    L3 --> R[返回响应]
    L4[运维侧 Secret 管理 错误脱敏 审计日志] -.贯穿全程.- L1
    L4 -.-> L2
    L4 -.-> L3
```

图 8-7：四层纵深防御分层。输入侧拦注入，执行侧管工具滥用，输出侧防泄漏，运维侧贯穿全程兜底。任何单层都可能被绕过，叠起来才有效。
...
把这套中间件串起来后，一条携带间接注入的请求会在哪一关被拦下，如图 8-8 所示——恶意指令藏在 Agent 抓取的网页里，规则和输入检测放行了正常的用户问句，但工具返回的内容在执行侧被注入检测拦截。
...
sequenceDiagram
    ...
    participant T as fetch_url 工具
    participant D as 注入检测器
    U->>M: 正常问句 帮我读这个链接
    M->>M: 清洗 + 规则 + LLM 检测 通过
    M->>A: 放行
    A->>T: 调用 fetch_url
...
图 8-8：间接注入被拦截的链路。用户输入本身合法，注入藏在工具检索到的外部内容里，在执行侧的工具出口被检测器拦下，模型读不到恶意指令。
...
下一节 [部署架构](./05-deployment-architecture.md) 把前面所有东西打包，讲单体 vs 微服务 vs Serverless 各自的取舍，以及监控告警怎么落地。
