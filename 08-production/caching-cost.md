...
> 模块 08 - 生产部署 | 前置知识：[Callback 系统](../07-observability/callbacks.md)、[API 服务化](./api-server.md)
...
要砍成本得先看清成本花在哪。结合 [Callback 系统](../07-observability/callbacks.md) 里讲的 `BillingHandler`，加一层告警：
...
下一节 [安全防御](./security.md) 讲 prompt injection、PII 脱敏、tool 权限沙箱这些生产 Agent 绕不开的安全话题。
