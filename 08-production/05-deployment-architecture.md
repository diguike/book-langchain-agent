---
title: 部署架构
feishu_url: "https://fivwvysqdz.feishu.cn/wiki/UNflwgQe9iHm77kPEgEcQIgqnSe"
last_synced: "2026-05-25T02:43:21+08:00"
---

> 模块 08 - 生产部署 | 前置知识：[API 服务化](./01-api-server.md)、[流式接口部署](./02-streaming-api.md)
...
适合单机 50 QPS 以内、内部工具、初创 MVP。前面挂个 Nginx 做 TLS + SSE 反代（参考 [流式接口部署](./02-streaming-api.md)）就能上线。
...
`proxy-buffering: off` 是 SSE 的命门，[流式接口部署](./02-streaming-api.md) 里讲过。
...
加一步**部署前跑核心 evaluator**（[评估方法与指标](../07-observability/03-evaluation.md)），分数跌过阈值直接红灯，避免坏 prompt / 坏 prompt cache 上线。
...
[Callback 系统](../07-observability/01-callbacks.md) 里讲过 `PromHandler` 的实现。Hono route：
...
模块 08 到这里结束。把前 8 个模块串起来——从 Runnable 抽象、Tool 定义、Agent 架构、RAG、可观测、到生产部署——一个完整的 LangChain.js Agent 项目就能落地。后续阅读建议看附录的 [API Cheatsheet](../appendix/02-api-cheatsheet.md) 做日常查阅。
