...
> 模块 08 - 生产部署 | 前置知识：[流式输出深入](../05-agent-architecture/10-stream-modes.md)、[API 服务化](./01-api-server.md)
...
[流式输出深入](../05-agent-architecture/10-stream-modes.md) 末尾已经给过一张对比表，这里把生产视角的决策再说一遍。
...
下一节 [缓存与成本优化](./03-caching-cost.md) 讲怎么用 semantic cache + prompt cache + 模型路由把 token 账单砍掉一半。
