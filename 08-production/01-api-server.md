...
- `runName` 在 LangSmith 上做 trace 名字（参考 [LangSmith Tracing](../07-observability/02-langsmith-tracing.md)）

## SSE 流式接口

流式接口是另一个文件，因为响应模式完全不同。详细 SSE 部署细节在 [流式接口部署](./02-streaming-api.md)，这里给最小实现：
...
**重要**：错误消息绝对不要透传给客户端原始内容。LLM provider 的错误里经常带 API Key 片段、上游 URL 等敏感信息，必须脱敏。详细的输出脱敏在 [安全防御](./04-security.md)。
...
下一节 [流式接口部署](./02-streaming-api.md) 展开讲 SSE 在 Nginx / Cloudflare 后面的生产部署细节，以及 WebSocket 什么时候用得上。
