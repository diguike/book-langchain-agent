...
前提是你在 Loader 阶段把 metadata 打全（见 [Document Loaders](./02-document-loaders.md)）。
...
| 极致精度 | k=20 + Rerank（见 [advanced-rag](./05-advanced-rag.md)） |
...
下一节 [高级 RAG](./05-advanced-rag.md) 在这套基础上叠 HyDE (Hypothetical Document Embeddings)、Rerank、Parent-Document、Query expansion 等更激进的技巧，专治 Naive RAG 治不了的问题。
