...
> 模块 06 - RAG | 前置知识：[Retriever 策略](./04-retrievers.md)
...
更彻底的做法是让 Agent 自己决定要不要拆、要拆成几个，见 [rag-agent](./06-rag-agent.md)。
...
完整的样本结构、判分 prompt、闭环代码和 LangSmith 集成，见 [评估方法与指标](../07-observability/03-evaluation.md)。专业评估框架（[Ragas](https://docs.ragas.io/)、[DeepEval](https://docs.confident-ai.com/)）也是这套思路，只是指标更细。
...
下一节 [RAG Agent](./06-rag-agent.md) 把所有这些技术从"固定管线"升级成"Agent 自主决策"——什么时候要检索、检索哪个库、要不要 rerank、置信度不够要不要回退到 web search，全部交给 LangGraph 状态机。
