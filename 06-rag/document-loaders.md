...
后面 [Retriever 策略](./retrievers.md) 章节展示的 metadata filter 全部依赖这套 schema。建议在 Loader 阶段就把它做对，后期补补丁很痛苦。
...
1. 加载时用 `TextLoader`，**Split 时**用 `MarkdownHeaderTextSplitter` 提取标题为 metadata（见 [Text Splitters](./text-splitters.md)）
...
下一节 [Text Splitters](./text-splitters.md) 讲怎么把这些 Document 切成大小合适的 chunk——分块策略直接决定召回率，是 RAG 调优最划算的环节。
